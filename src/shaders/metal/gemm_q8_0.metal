...
// Q8_0 GEMM kernel for prefill validation.
//
// Computes dst[N, M] = W[M, K] x src[N, K] with Q8_0 weights and f32 input.
// Tile shape and output layout match gemm_q4k/gemm_q6k: 64 rows x 32 tokens.
...
constant constexpr int NL0 = 2;
constant constexpr int NL1 = 4;
...
kernel void main0(
    ...
    const ulong row_bytes = args.nb01;
    device const uchar * row = (device const uchar *)(src0 + args.src0_off + row_bytes * (r0 + lr0));
    ...
    for (int loop_k = 0; loop_k < args.ne00; loop_k += NK) {
        device const uchar * block = row + (ulong(loop_k >> 5) * 34ull);
        ...
        FOR_UNROLL (short i = 0; i < 16; i++) {
            ...
            *(sa + 64 * ib + 8 * ly + lx) = half(scale * float(qs[q_idx]));
        }

...
// K=4096 specialization derived from dmmv_q4k_lmhead.metal. The MAX_K_VEC4
// bump from 768 to 1024 raises the threadgroup activation cache from 12 KiB
// to 16 KiB, which fits within Apple7's 32 KiB threadgroup memory budget.
// ROWS_PER_TG=8 (TG_SIZE=256) keeps occupancy near the M1 Max sweet spot:
// at K=4096 the wider TG (TG_SIZE=512, 16 rows) used in the K<=3072 variant
// over-allocates threads and hurts SM occupancy. Routed for Qwen3 attn_o and
// attn_v, where K=4096 is the bottleneck and M ∈ {1024, 4096} provides
// enough TGs to fill the GPU.
#define TG_SIZE 256
